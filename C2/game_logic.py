...
class QuoridorGame:
    def __init__(self):
        ...
        self.red_pos = (0, 4)
        self.blue_pos = (8, 4)
        self.remain_walls = {"red" : 8, "blue": 8} # 每个玩家剩余墙数量
    ...
    def can_reach_end(self, piece):
        q = [self.red_pos if piece == "red" else self.blue_pos]
        vis = set(q[0])
        while len(q) > 0:
            row, col = q.pop()
            if (piece == "red" and row == BOARD_SIZE - 1) or (piece == "blue" and row == 0):
                return True
            for i in range(4):
    ...
    def check_place_horizontal_wall(self, row, col):
    ...
    def check_place_vertical_wall(self, row, col):
        if self.remain_walls[self.player] == 0:
            return False
    ...
    def place_horizontal_wall(self, row, col):
        if not self.check_place_horizontal_wall(row, col):
            return False
        self.walls[(row, col)][0] = 1
        self.walls[(row - 1, col)][1] = 1
        self.walls[(row, col + 1)][0] = 1
        self.walls[(row - 1, col + 1)][1] = 1
        self.remain_walls[self.player] -= 1
        return True

    def place_vertical_wall(self, row, col):
        if not self.check_place_vertical_wall(row, col):
            return False
        self.walls[(row, col)][2] = 1
        self.walls[(row, col - 1)][3] = 1
        self.walls[(row + 1, col)][2] = 1
