...
class Trainer:
    ...
    def exceute_episode(self):
        ...
        while True:
            ...
            vis_state.add(tuple(root.state))
            ...
            action = root.select_action(temperature=10)
            state, current_player = self.game.get_next_state(state, current_player, action)
            reward = self.game.get_reward_for_player(state, current_player)

            if reward is not None or tuple(state) in vis_state:
                if tuple(state) in vis_state:
                    reward = -1
                ret = []
    ...
    def learn(self):
        for i in range(1, self.args['numIters'] + 1):
            ...
            for eps in range(self.args['numEps']):
                print (f"play games: {eps}/{self.args['numEps']}")
