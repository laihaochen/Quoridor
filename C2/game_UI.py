...
class QuoridorUI:
    ...
    def reset(self):
        self.game.reset()
        self.current_mode = "move_piece"
        self.canvas.delete("preview_piece")
        for wall in self.canvas_walls:
            self.canvas.delete(wall)
        self.canvas_walls = []
        self.canvas.coords(self.piece_red, 4 * CELL_SIZE + 10, 10, 5 * CELL_SIZE - 10, CELL_SIZE - 10)
        self.canvas.coords(self.piece_blue, 4 * CELL_SIZE + 10, 8 * CELL_SIZE + 10, 5 * CELL_SIZE - 10, 9 * CELL_SIZE - 10)
    ...
    def draw_start_frame(self):
        ...
        example_image1 = example_image1.resize((200, 200), Image.ANTIALIAS)  
        ...
        example_image2 = example_image2.resize((200, 200), Image.ANTIALIAS)  
    ...
    def draw_circle(self, row, col, color, tag): # 在 (row, col) 处绘制颜色为 color 的圆圈
        x = row * CELL_SIZE
        y = col * CELL_SIZE
        return self.canvas.create_oval(y + 10, x + 10, y + CELL_SIZE - 10, x + CELL_SIZE - 10, fill=color,outline=color, tag=tag)
    
    def draw_piece(self):
        self.piece_red = self.draw_circle(0, BOARD_SIZE // 2, "red", "piece")
        self.piece_blue = self.draw_circle(BOARD_SIZE - 1, BOARD_SIZE // 2, "blue", "piece")
    ...
    def draw_control_board(self):
        ...
        # 墙放置模式选择
        self.button_place_horizontal_wall = tk.Button(self.control_board, text="放置横向墙", command=lambda: self.set_mode("place_horizontal_wall"))
        self.button_place_horizontal_wall.grid(row=0, column=0)

        self.button_place_vertical_wall = tk.Button(self.control_board, text="放置竖向墙", command=lambda: self.set_mode("place_vertical_wall"))
        self.button_place_vertical_wall.grid(row=0, column=1)

        # 移动模式选择
        self.button_move_mode = tk.Button(self.control_board, text="移动", command=lambda: self.set_mode("move_piece"))
    ...
    def draw_reachable_positions(self):
        ...
        for position in reachable_positions:
            if self.game.player == "red":
                self.draw_circle(position[0], position[1], "salmon", "preview_piece")
            else:
                self.draw_circle(position[0], position[1], "deepskyblue", "preview_piece")

    # 选择模式函数
    def set_mode(self, mode):
        self.current_mode = mode
        if self.current_mode == "move_piece":
            self.label_now_mode.config(text="当前模式：移动")
        elif self.current_mode == "place_horizontal_wall":
    ...
    def change_player(self):
        ...
        self.set_mode("move_piece")

    # 禁用按钮
    def disable_button(self):
        self.button_move_mode.config(state="disabled")
        self.button_place_horizontal_wall.config(state="disabled")
        self.button_place_vertical_wall.config(state="disabled")

    # 启用按钮
    def enable_button(self):
        self.button_move_mode.config(state="normal")
        self.button_place_horizontal_wall.config(state="normal")
        self.button_place_vertical_wall.config(state="normal")

    # 游戏结束
    def game_end(self):
        self.disable_button()
        self.root.after(300, lambda: None)
        self.game_over_text = self.canvas.create_text(270, 250, text=f"{self.game.player} wins!", font=("Arial", 50), anchor="center", fill=self.game.player)
    ...
    def show_wall_preview(self, event):
        ...
        # 先清除之前的提示
        # 根据当前模式显示墙的位置
        if self.current_mode == "place_horizontal_wall":
            row = (y + CELL_SIZE // 2) // CELL_SIZE
            col = (x - CELL_SIZE // 2) // CELL_SIZE
            if self.game.check_place_horizontal_wall(row, col):
                self.canvas.create_line(
                    col * CELL_SIZE, row * CELL_SIZE,
                    (col + 2) * CELL_SIZE, row * CELL_SIZE, width=5, fill="gray", tags="preview_wall"
                )
        elif self.current_mode == "place_vertical_wall":
            row = (y - CELL_SIZE // 2) // CELL_SIZE
            col = (x + CELL_SIZE // 2) // CELL_SIZE
            if self.game.check_place_vertical_wall(row, col):
                self.canvas.create_line(
                    col * CELL_SIZE, row * CELL_SIZE,
    ...
    def move_piece(self, row, col):
        now_peice = self.piece_red if self.game.player == "red" else self.piece_blue
        if self.game.move_piece(row, col):
            self.canvas.delete("preview_piece")
            self.canvas.coords(now_peice, col * CELL_SIZE + 10, row * CELL_SIZE + 10, (col + 1) * CELL_SIZE - 10, (row + 1) * CELL_SIZE - 10)
            return True
        return False

    # 移动棋子
    def mouse_move_piece_action(self, event):
        if self.game.is_end():
            return None
    ...
    # 放置横向墙
    def place_horizontal_wall(self, row, col):
        if self.game.place_horizontal_wall(row, col):
    ...
    def mouse_place_horizontal_wall_action(self, event):
        x, y = event.x, event.y
        row = (y + CELL_SIZE // 2) // CELL_SIZE
        col = (x - CELL_SIZE // 2) // CELL_SIZE
    ...
    # 放置竖向墙
    def place_vertical_wall(self, row, col):
        if self.game.place_vertical_wall(row, col):
            self.canvas_walls.append(self.canvas.create_line(
                col * CELL_SIZE, row * CELL_SIZE,
    ...
    def mouse_place_vertical_wall_action(self, event):
        x, y = event.x, event.y
        row = (y - CELL_SIZE // 2) // CELL_SIZE
        col = (x + CELL_SIZE // 2) // CELL_SIZE
    ...
    def get_action_from_mouse(self, event):
        ...
        action = None
        if self.current_mode == "move_piece":
            action = self.mouse_move_piece_action(event)
        elif self.current_mode == "place_horizontal_wall":
            action = self.mouse_place_horizontal_wall_action(event)
        elif self.current_mode == "place_vertical_wall":
            action = self.mouse_place_vertical_wall_action(event)
    ...
    def get_next_game(self, action):
        ...
        if action < BOARD_SIZE * BOARD_SIZE:
            ...
            successful_move = self.move_piece(row, col)
        elif action < 2 * BOARD_SIZE * BOARD_SIZE:
            self.set_mode("place_horizontal_wall")
            action -= BOARD_SIZE * BOARD_SIZE
            row = action // BOARD_SIZE
            col = action % BOARD_SIZE
            successful_move = self.place_horizontal_wall(row, col)
        else:
            self.set_mode("place_vertical_wall")
            action -= 2 * BOARD_SIZE * BOARD_SIZE
            row = action // BOARD_SIZE
            col = action % BOARD_SIZE
            successful_move = self.place_vertical_wall(row, col)
        if self.game.is_end():
            self.game_end()
        if successful_move:
            self.change_player()
